...
**全局唯一字体栈**（定义于 `src/index.css :root`，禁止模块级覆盖 font-family）：

```
'PingFang SC', 'Helvetica Neue', Helvetica, 'Segoe UI', Arial, sans-serif
```

- 全项目统一 PingFang SC；仅代码展示场景允许 Consolas / Menlo 等等宽字体；
- 禁止任何模块 css 以 `font-family: Arial / Microsoft YaHei` 等覆盖全局栈。

字号层级：

| Token | 值 | 场景 |
|---|---|---|
| `--fs-title` | 17px | 页面标题 / 抽屉标题 / 侧栏品牌 |
| `--fs-lg` | 16px | 弹窗标题、模块强调标题 |
| `--fs-md` | 15px | 卡片标题、表格强调、次级标题 |
| `--fs-base` | 14px | **正文默认**：表格单元格、菜单、表单 |
| `--fs-aux` | 12px | 辅助：提示、标签、时间、图例 |

字重层级（仅允许三档，**禁用 700/800/900**：PingFang SC 无该字重，浏览器合成假粗体发丑）：

| Token | 值 | 场景 |
|---|---|---|
| `--fw-regular` | 400 | 正文、表格单元格内容、按钮与操作链接、表单控件、描述文字 |
| `--fw-medium` | 500 | 表头、选中/激活/勾选态、标签徽章、列表主文（商品名等）、kv 键 |
| `--fw-semibold` | 600 | 标题（页面/卡片/弹窗/抽屉/分组）、品牌 LOGO 字、数字强调（KPI/统计值） |

规则：
- 操作类文字（按钮、表内操作链接、分页、刷新等）一律 400，不加粗；
- 状态强调用 500 + 主色表达，不直接上 600 及以上；
- 数字强调（KPI 值、统计值）用 600，不用 800。
...
- 表头 / 单元格统一 `--fs-base`；表头 `--fw-medium` + 浅字色，单元格 `--color-text-2`；
